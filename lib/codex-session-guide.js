...
function generateCodexGuide(language = 'typescript', options = {}) {
  ...
  const listExamples =
    Array.isArray(files.shared) ||
    Array.isArray(files.language) ||
    Array.isArray(files.local) ||
    Array.isArray(files.lint);
  ...
  lines.push('- `.dev/lint/*.md` (lint checklists & IDE parity notes)');
  lines.push('');
  ...
  if (listExamples || Array.isArray(files.lint)) {
    lines.push('');
    ...
    (files.lint || []).slice(0, 1).forEach(p => sample.push(`- ${p}`));
